...
#include "leak.h"
#endif
flickrImgSearchQueryDispatcher::flickrImgSearchQueryDispatcher()
{
	m_uNumThreads=2;
	m_bAbort=false;
	m_bHttpError=false;
...
void flickrImgSearchQueryDispatcher::run()
{
	...
	while(m_QueryThreads.size()<numThreads){
		flickrImgSearchQueryThread* p=new flickrImgSearchQueryThread;
		p->setQuery(m_Query);
...
void flickrImgSearchQueryDispatcher::childFinished()
{
	// mutual exclusive...
	m_Mutex.lock();
